...
# 2. Базові метрики якості: Використати SonarQube й інші інструменти для аналізу метрик коду. 
## 1. **Security (Безпека)**
   - **Відкритих проблем:** 0
   - **Security Hotspots (Точки безпеки):** 0
   - **Оцінка безпеки:** Проблеми безпеки не виявлені, і в коді немає критичних точок, які потребують уваги.

## 2. **Reliability (Надійність)**
   - **Відкритих проблем:** 0
   - **Оцінка надійності:** У коді не виявлено жодних питань щодо надійності. Код працює стабільно і не містить уразливих місць.

## 3. **Maintainability (Підтримуваність)**
   - **Відкритих проблем:** 0
   - **Оцінка підтримуваності:** Код добре структурований, без зайвих складнощів, що забезпечує легку підтримку й модифікацію. Не виявлено проблем, що можуть ускладнити підтримку коду в майбутньому.

## 4. **Coverage (Покриття тестами)**
   - **Необхідні додаткові кроки для налаштування аналізу покриття тестами.**
   - Для повної оцінки покриття тестами потрібно налаштувати аналіз покриття для SonarQube Cloud. Це дозволить визначити, яку частину коду покривають тести.

## 5. **Duplications (Дублювання)**
   - **Процент дублювання:** 0.0%
   - Код не містить дубльованих частин, що свідчить про високий рівень оптимізації й уникання повторень.

## Загальний висновок:
Аналіз коду за допомогою SonarQube показує, що код не має проблем із безпекою, надійністю, підтримуваністю та дублюванням. Однак для повного аналізу тестового покриття необхідно налаштувати додаткові кроки для аналізу покриття тестами в SonarQube Cloud.
![image_2025-03-16_12-56-13](https://github.com/user-attachments/assets/02e7734a-7f65-4f68-99b5-8cdb25c4077c)
...
## 1. Використати Git Flow для управління гілками розробки (для фіч, багів, релізів).
![image_2025-03-16_13-26-16](https://github.com/user-attachments/assets/9fce87d0-8810-4e92-a169-0436d38ce505)

## 2. Налаштувати GitHub Actions для автоматичного запуску модульних тестів.
![image_2025-03-16_17-14-27](https://github.com/user-attachments/assets/3bae736e-e092-4f65-bb8b-93512087f426)

## 3-4. Автоматизувати перевірку коду перед злиттям у основну гілку.
![image_2025-03-16_17-46-00](https://github.com/user-attachments/assets/4a817cbe-7e5c-4bc4-a306-8f53e61612b9)
![image_2025-03-16_18-16-04](https://github.com/user-attachments/assets/a8054642-76e7-42de-a99c-036b61b34114)
![image_2025-03-16_18-17-05](https://github.com/user-attachments/assets/23e8e875-1f0f-42a5-8cb5-6940887a0a9c)
